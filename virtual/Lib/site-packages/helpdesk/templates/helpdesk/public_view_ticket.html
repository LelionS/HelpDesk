    ...
    <div class="card shadow-lg rounded-lg border-0">
      <div class="card-header bg-dark text-white d-flex justify-content-between align-items-center">
        <h5 class="mb-0">
          ...
          <span class="badge badge-info ml-2">{{ ticket.get_status }}</span>
        </h5>
      </div>
      ...
      <div class="card-body p-0">
        <table class="table table-striped table-hover mb-0">
          <tbody>
            <tr>
              <th style="width: 30%">{% trans "Queue" %}</th>
              <td>{{ ticket.queue }}</td>
            </tr>
            ...
            <tr class="table-success">
              <th colspan="2" class="d-flex justify-content-between align-items-center">
                <span>{% trans "Resolution" %}</span>
                {% if ticket.get_status_display == "Resolved" %}
                <a href='{{ ticket.ticket_url }}&close'>
                  <button type="button" class="btn btn-success btn-sm">{% trans "Accept and Close" %}</button>
                </a>
                {% endif %}
              </th>
            </tr>
